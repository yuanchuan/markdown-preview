...
var server = http.createServer(handler)
  , countdownExit = countdown(function() {
      process.exit(1);
    }, 4e3);  
...
var handler = function(req, res) {
  ...
  check(filename, function(ok) {
    if (ok) {
      ...
      response(res, 404, 'text', 
        'Cannot open file ' + basename
      );
    }
  });
}
...
exports.startAt = function(port, fn) {
  http
